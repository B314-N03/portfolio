...
import { faClose, faHome, faMinus, faPlus, faTerminal } from '@fortawesome/free-solid-svg-icons';
...
function Home(): JSX.Element {
  ...
  const ageOfMe = age   
  const typeCommand = async (commandText: string, outputTextArray: string[], styles = {commandColor:"",commandSize:"",outputColor:"",outputSize:""}) => {
    const { commandColor, commandSize, outputColor, outputSize } = styles;
    const currentLine = { command: "", output: "", styles: {} };
    ...
    setTerminalLines((prevLines) => [...prevLines, { command: "", output: "", styles: {} }]);
  };
const exec = async () => {
 // Enhanced function to include styles
  ...
  const techStack: { [key: string]: { name: string; icon?: IconProp; iconClassName?: string; svg?: boolean; svgPath?: string,svgClassName? : string } } = {
    "react_js":{
      name:"React.js",
      icon:faReact,
  ...
  const handleCommandSubmit : React.FormEventHandler<HTMLFormElement>  = (e) => {
    ...
    if (currentCommand.trim()) {
      ...
      setTerminalLines((prevLines) => [
        ...prevLines,
        { command: currentCommand, output,styles:{} },
      ]);
        ...
        {siteType !== "static" && <div>
          {!showTerminal && 
                <div className="flex-column flex-gap-5p justify -center align-center w-fit-content position-absolute cursor-pointer" onClick={handleOpenNewTerminal}>
                  <FontAwesomeIcon icon={faTerminal as IconProp} className="h4 home-profileCard-header-terminal-icon "></FontAwesomeIcon>
              ...
              <div className="home-profileCard-header">
                <div className="home-profileCard-header-icon-container">
                  <div className="bg-color-red terminal-header-icon" onMouseEnter={() => handleMouseEnterTerminalIcon("terminal-icon-red")} onClick={() => setShowTerminal(!showTerminal)} onMouseLeave={() => handleMouseLeaveTerminalIcon("terminal-icon-red")}>
                    <FontAwesomeIcon icon={faClose as IconProp} fontSize={"14px"} id="terminal-icon-red" ></FontAwesomeIcon>
                  </div>
                  <div className="bg-color-yellow text-color-black terminal-header-icon" onMouseEnter={() => handleMouseEnterTerminalIcon("terminal-icon-yellow")} onMouseLeave={() => handleMouseLeaveTerminalIcon("terminal-icon-yellow")}>
                    <FontAwesomeIcon icon={faMinus as IconProp} fontSize={"14px"} id="terminal-icon-yellow"></FontAwesomeIcon>
                  </div>
                  <div className="bg-color-green terminal-header-icon" onMouseEnter={() => handleMouseEnterTerminalIcon("terminal-icon-green")} onMouseLeave={() => handleMouseLeaveTerminalIcon("terminal-icon-green")}>
                    <FontAwesomeIcon icon={faPlus as IconProp} fontSize={"14px"} id="terminal-icon-green"></FontAwesomeIcon>
                  </div>
                </div>
                ...
                <span className="h4 home-profileCard-header-terminal-icon">
                  <FontAwesomeIcon icon={faTerminal as IconProp}></FontAwesomeIcon>
                </span>
              </div>
      ...
      {/* Content for static site */}
        {siteType === "static" && 
          <div className='home-flex-container z-index-1 w-100 align-center text-color-main'>
            <div className="flex-column flex-gap-5p w-50-none-mobile padding-l-10">
              <div id="text-1" className="font-size-huge animate__animated visibility-hidden">Béla Noé</div>
              <div id='text-2' className="font-size-huge animate__animated visibility-hidden">Software Entwickler</div>
              <div id="text-3" className="font-size-huge animate__animated visibility-hidden">7 Jahre Erfahrung</div>
            </div>

            <GlassCard bodyClassName='padding-30 flex-column flex-gap-10p' containerClassName='glass-card-home' windowIcon={faHome as IconProp} windowTitle=''>
                      ...
                      <div className="flex-row techstack-container  animate__animated animate__fadeInDown">
                          ...
                          Object.keys(techStack).map((entry) => {
                            if(techStack[entry].svg){
                              ...
                              return(
                                <div className="techstack-child-container" key={entry}>
                                  <FontAwesomeIcon icon={techStack[entry].icon as IconProp} className={`techstack-child-icon${techStack[entry].iconClassName !== undefined ? " " + techStack[entry].iconClassName : ''}`}></FontAwesomeIcon>
                                  <span className='techstack-child-name'>{techStack[entry].name}</span>
                                </div>
                              )
                            }
